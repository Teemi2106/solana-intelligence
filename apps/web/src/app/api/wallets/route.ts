import { z } from "zod";
import { createTrackedWallet, listTrackedWallets, startWalletIngestion } from "@swi/db";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!parsed.success) return Response.json({ error: "invalid wallet", issues: parsed.error.issues }, { status: 400 });

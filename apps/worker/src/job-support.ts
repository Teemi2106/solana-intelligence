  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => {
          reject(new JobTimeoutError(label));
        }, milliseconds);
      }),
    ]);
  } finally {
...
export async function recordJobFailure(
  ...
  await database.query
    ...
    .values({
      ...
      jobId: job.id,
      providerEventId,
      errorCode: error.name.slice(0, 64),
    ...
    .onConflictDoUpdate({
      target: [
        schema.processingFailures.queue,
        schema.processingFailures.jobId,
      ],
      set: {
        errorCode: error.name.slice(0, 64),
        errorMessage: error.message.slice(0, 500),
  ...
  if (providerEventId)
    await database.query
      ...
      .set({ status: "FAILED", lastErrorCode: error.name.slice(0, 64) })
